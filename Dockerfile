...
RUN mkdir -p /media/storage && chmod 777 /media/storage
RUN mkdir -p /media/storage/spatial_transcriptomics_output && chmod 777 /media/storage/spatial_transcriptomics_output
...
RUN mkdir cidc_spatial_transcriptomics
WORKDIR /home/pipeline/cidc_spatial_transcriptomics

#Copy in all of the files/directories/etc.. from the project into the container
COPY . .
USER root
RUN chown -R pipeline:pipeline /home/pipeline/cidc_spatial_transcriptomics
USER pipeline
...
#RUN conda activate cidc_spatial_transcriptomics

#ENTRYPOINT ["conda", "activate", "cidc_spatial_transcriptomics"]
