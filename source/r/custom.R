...
## Load the annotation categories/levels from TXT file
lvls <- readLines(snakemake@input$txt)
print(lvls)

refdata <- setNames(as.list(lvls), paste0("custom_", lvls))
print(refdata)
...
for (i in seq_along(lvls)) {
	    lvl <- lvls[i]
    cell <- paste0("predicted.custom_", lvl)
    score <- paste0("predicted.custom_", lvl, ".score")

    ## Rename the annotation columns to include the sample type
    cell.rename <- paste("custom", lvl, sep="_")
    score.rename <- paste0("custom_", lvl, ".score")
...
## Save the annotation categories/levels for use in the report
